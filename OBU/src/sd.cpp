...
static bool parse_line(const String &line, AccidentData &d, int &sent) {
  ...
  d.second               = ji(line, "ss");
  sent                   = ji(line, "sent");
  // vehicleId is a string — extract it from the JSON
  {
    String key = "\"vid\":\"";
    ...
    if (idx >= 0) {
      ...
      if (end > start) {
        String vid = line.substring(start, end);
        strncpy(d.vehicleId, vid.c_str(), sizeof(d.vehicleId) - 1);
        d.vehicleId[sizeof(d.vehicleId) - 1] = '\0';
      }
    }
  }
  return true;
